...
DEFAULT_COLOURS = {' ': [0, 0, 0],  # Black background
                   ...
                   'm': [255, 222, 173],  # Bed
                   'n': [255, 222, 173],  # Bed
                   'S': [95, 158, 160],  # Blue nurse station
                   '@': [105, 105, 105], #Grey walls
                   'T': [188,143,143], #Toilet
...
class MapEnv(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        for agent in self.agents.values():
            pos = agent.get_pos()
            new_char = agent.complete(self.world_map[pos[0], pos[1]], self.pos_dict)
    ...
    def render(self, filename=None):
        ...
        # my_dpi=96
        my_dpi = fig.get_dpi()
        fig.set_size_inches(288.0 / float(my_dpi), 384.0 / float(my_dpi))
